...
async fn handler(event: CloudWatchEvent) -> Result<(), Box<dyn Error>> {
    if let Some(request) = event.detail.request_data {
        let operation = request.http_method.as_str();
        ...
        let parsed_body: Value = match serde_json::from_str(&request.body) {
            Ok(value) => value,
            Err(_) => {
                error!("Invalid JSON payload: {}", request.body);
                return Ok(Response::builder()
        ...
        match client.get_item(input).await {
            Ok(response) => {
                match response.item {
                    Some(item) => {
                        ...
                        info!("Item found with URL: {}", object_url);
                        Ok(Response::builder()
                            .status(StatusCode::OK)
                            .header("Content-Type", "application/json")
                    ...
                    None => {
                        error!("Item not found for barName: {} and drinkName: {}", bar_name, drink_name);
                        Ok(Response::builder()
            ...
            Err(_) => {
                error!("Failed to query DynamoDB for barName: {} and drinkName: {}", bar_name, drink_name);
                Ok(Response::builder()
        ...
        error!("No request data found in CloudWatch event");
        Ok(())
    }
}
